...
function createTest() {
  ...
  it("should return string with a url by promise style", (done) => {
    let home = Promise.resolve(app.render("/")).then(({ content }) => {
      expect(content).toEqual("home");
    });
    let list = Promise.resolve(app.render("/list")).then(({ content }) => {
      expect(content).toEqual("list");
    });
    let detail = Promise.resolve(app.render("/detail")).then(({ content }) => {
      expect(content).toEqual("detail");
    });
    let notfound = Promise.resolve(app.render("/notfound")).then(
      ({ content }) => {
        expect(content).toEqual("not found");
      }
    );
    let random = Promise.resolve(
      app.render(`/random-${Math.random().toString(36).substr(2)}`)
    ).then(({ content }) => {
      expect(content).toEqual("not found");
    });
  ...
  it("should support inject context to app.render method", (done) => {
    let home = Promise.resolve(app.render("/", { test: 1 })).then(
      ({ content, controller }) => {
    ...
    let list = Promise.resolve(app.render("/list", { test: 2 })).then(
      ({ content, controller }) => {
        if (controller && controller.context) {
